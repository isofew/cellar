...
A simple dns proxy to tunnel DNS requests over a socks proxy (for 
example, over ssh or Tor). This can come in handy when setting up 
transparent proxies.

It chooses a random DNS server for each request from the file 
"resolv.conf" which is a newline delimited list of DNS servers. 
...
With no parameters, the configuration file is read from 
'dns_proxy.conf'.

* -n          -- No configuration file (socks: 127.0.0.1:9050, listener: 
0.0.0.0:53).
...
The configuration file should contain any of the following options (and 
...
* log_file    -- location to log to (should be /dev/null unless 
debugging).
...
On Arch linux, the dns proxy can be installed from the AUR at: 
https://aur.archlinux.org/packages/tcpdnsproxy/.
...
After running the daemon, the system should be configured to use the 
proxy automatically.
